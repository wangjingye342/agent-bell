...
2. **自己发 mDNS 组播查询**（UDP 5353 问 `agent-bell.local`），实测 0.2~0.3 秒命中
3. 并发扫描网段（先扫缓存 IP 所在的 /24，再扩到其它网卡；强判定 `app == "agent-bell"`）

> 为什么不用 `gethostbyname("agent-bell.local")`：装了 Clash 一类 TUN 代理时，`.local`
> 会被 fake-ip 劫持到 198.18.0.0/15 然后被 RST（实测 0.02 秒就返回失败）——系统 DNS
> 这条路是死的，而设备自己的 mDNS 应答是好的，所以绕过系统解析直接发组播。

> 扫描排除这些无关网段：198.18.0.0/15（TUN fake-ip）、100.64.0.0/10（CGNAT/Tailscale）、
> 169.254/16（link-local）、172.17/16（Docker）。扫它们纯浪费，还会把真实局域网的探测挤掉。

**换路由器 / DHCP 重分配后**：缓存 IP 失效 → mDNS 组播接上，实测 3.3 秒自愈
（改之前要 ~90 秒、扫三轮才命中）。
